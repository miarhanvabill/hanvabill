...
import { getAuthenticatedSql, getInternalTenantId } from "@/lib/db"
import { revalidatePath } from "next/cache"
import { withTenantAuth } from "@/lib/auth-server"

export async function getMiniWebsiteSettings(tenantKey: string) {
  try {
...
export const saveMiniWebsiteSettings = withTenantAuth(async (tenantKey: string, formData: any) => {
  try {
    const { sql, tenantId } = await getAuthenticatedSql(tenantKey)
