...
export async function GET(request: NextRequest) {
  try {
    ...
    if (p.get("transactions") === "1") {
      ...
      const { rows, total } = await getLoyaltyTransactions({ customer_id: customerId, type, from, to, limit, offset })
    ...
    const [data, settings] = await Promise.all([getCustomerLoyalty(id), getLoyaltySettings()])
    const days = Number(p.get("expDays") || 7)
    const expiringSoon = data ? await getExpiringSoon(id, days) : 0

    return NextResponse.json({ success: true, data, settings, expiringSoon: { days, points: expiringSoon } })
  } catch (e: any) {
