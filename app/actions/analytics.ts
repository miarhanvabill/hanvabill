...
export async function getBusinessAnalytics(dateRange: string): Promise<AnalyticsData> {
  ...
  try {
    ...
    const currRevResult = await sql`
      ...
      FROM bookings
      WHERE booking_date BETWEEN ${startIso} AND ${endIso}
    ...
    const prevRevResult = await sql`
      SELECT COALESCE(SUM(total_amount),0) AS total
      FROM bookings
      WHERE booking_date BETWEEN ${prevIso} AND ${startIso}
      AND status='completed'
    `
    ...
    const currCustResult = await sql`
      ...
      FROM customers
      WHERE created_at BETWEEN ${startIso} AND ${endDate.toISOString()}
    `
    ...
    const prevCustResult = await sql`
      SELECT COUNT(*) AS count
      FROM customers
      WHERE created_at BETWEEN ${prevIso} AND ${startIso}
    `
    ...
    const currBookResult = await sql`
      SELECT COUNT(*) AS count
      FROM bookings
      WHERE booking_date BETWEEN ${startIso} AND ${endDate.toISOString()}
      AND status='completed'
    `
    ...
    const prevTimeResult = await sql`
      ...
      JOIN booking_services bs ON b.id = bs.booking_id
      JOIN services s ON bs.service_id = s.id
      WHERE b.booking_date BETWEEN ${prevIso} AND ${startIso}
      AND b.status='completed'
    `
    ...
    const repeatCustResult = await sql`
      SELECT COUNT(DISTINCT customer_id) FILTER (
        WHERE customer_id IN (
          SELECT customer_id FROM bookings WHERE booking_date < ${startIso}
        )
      ) AS repeat_count
      FROM bookings
      WHERE booking_date >= ${startIso}
    ...
    const utilRes = await sql`
      ...
      FROM staff st
      JOIN bookings b ON st.id = b.staff_id
      JOIN booking_services bs ON b.id = bs.booking_id
      JOIN services s ON bs.service_id = s.id
      WHERE b.booking_date >= ${startIso}
      AND b.status='completed'
