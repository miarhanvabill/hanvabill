...
export async function sendWhatsAppMessage(data: {
  ...
  try {
    ...
    const customer = await sql`
      SELECT id FROM customers WHERE phone_number = ${data.phoneNumber}
    `

    await sql`
      INSERT INTO whatsapp_messages (
        customer_id, phone_number, message_type, message_content, direction, status
      ) VALUES (
        ${customer[0]?.id || null}, ${data.phoneNumber}, ${data.messageType}, 
        ${data.message}, 'outbound', 'sent'
      )
    `
