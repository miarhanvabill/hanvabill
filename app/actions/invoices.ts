...
export interface InvoiceData {
  ...
  due_date: string
  notes?: string | null
  metadata?: any | null // NEW: extras like coupon, loyalty, gift cards, rounding, savings, etc.
}
...
export async function createInvoice(data: InvoiceData) {
  try {
    ...
    // Try with metadata/share_token if columns exist; fallback without
    try {
      const rows = await sql`
        INSERT INTO invoices (
          invoice_number, customer_id, amount, subtotal, discount_amount, gst_amount,
          payment_method, service_details, product_details, invoice_date, due_date,
          notes, metadata, share_token, created_at, updated_at
        ) VALUES (
          ${invoiceNumber}, ${data.customer_id}, ${data.amount}, ${data.subtotal}, ${data.discount_amount}, ${data.gst_amount},
...
export async function ensureShareToken(id: string) {
  try {
    const rows = await sql`SELECT share_token FROM invoices WHERE id = ${id} LIMIT 1`
    const existing = rows?.[0]?.share_token
    if (existing) return { success: true, share_token: existing }
    const token = genShareToken()
    try {
      const u = await sql`UPDATE invoices SET share_token = ${token}, updated_at = NOW() WHERE id = ${id} RETURNING share_token`
