...
const loyaltyBlockRegex = /        \n        \{data\.loyaltyPointsAvailable !== undefined && data\.loyaltyPointsEarned !== undefined && \(data\.loyaltyPointsAvailable > 0 \|\| data\.loyaltyPointsEarned > 0\) && \([\s\S]*?<\/div>\n        \)\}\n      <\/div>/;

code = code.replace(loyaltyBlockRegex, `      </div>`);

// 2. Add it to the bottom right after Notes/Terms and Totals
const bottomTarget = `        <div className="w-full md:w-72 space-y-3">`;
const bottomReplacement = `        <div className="w-full md:w-72 space-y-3">`;
...
code = code.replace(
  ...
  `
      ...
      {data.loyaltyPointsAvailable !== undefined && data.loyaltyPointsEarned !== undefined && (data.loyaltyPointsAvailable > 0 || data.loyaltyPointsEarned > 0) && (
        <div className="mt-8 bg-slate-50 p-4 rounded-lg border border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-sm print:break-inside-avoid">
          <div>
